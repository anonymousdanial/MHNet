...
if __name__ == "__main__":
    # Small runnable test: construct dummy model outputs and targets and run the
    # loss to ensure the class computes values without syntax errors.
    torch.manual_seed(0)
    ...
    # Random logits and boxes (cx, cy, w, h) in [0,1]
    outputs = {
        'pred_logits': torch.randn(batch_size, num_queries, num_classes + 1),
        'pred_boxes': torch.rand(batch_size, num_queries, 4)
    }

    # Build targets: list of dicts with variable number of objects per image
    targets = []
    for i in range(batch_size):
        n_obj = int(torch.randint(1, 4, (1,)).item())
        labels = torch.randint(0, num_classes, (n_obj,), dtype=torch.int64)
        # Ensure widths/heights are positive and reasonable
        boxes = torch.rand(n_obj, 4)
        boxes[:, 2:] = boxes[:, 2:] * 0.5 + 0.05
        targets.append({'labels': labels, 'boxes': boxes})

    if linear_sum_assignment is None:
        print('Warning: scipy.optimize.linear_sum_assignment not available. Hungarian matching will fail if used.')
